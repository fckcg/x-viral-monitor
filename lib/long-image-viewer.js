...
(() => {
  ...
  function init() {
    ...
    // When URL changes we schedule THREE checks at staggered delays — the
    // carousel transform takes a couple hundred ms to settle, and during
    // that window getActiveMediaImg() returns null because the target slide
    // is still off-screen. Without the retries LIV would stay deactivated
    // permanently for the new slide. Retry windows chosen to cover both
    // short snap-back (250ms) and slower system / animation budgets (600ms).
